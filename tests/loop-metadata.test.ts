...
describe("Loop metadata generation", () => {
  test("adds dynamic IDs and data source metadata for static in-file collections", () => {
    ...
    expect(output).toMatch(
      /<span[^>]*data-editor-id="[a-f0-9]{12}"[^>]*data-children-source=\{\s*\[[\s\S]*?\{\\"file\\":\\"Example-static\.jsx\\",\\"start\\":\\"\d+:\d+\\",\\"end\\":\\"\d+:\d+\\",\\"id\\":\\"[^"]+\\"\}[\s\S]*?\]\s*\[index\]\s*\}/,
    );
    expect(output).toMatch(
      /<p[^>]*data-editor-id="[a-f0-9]{12}"[^>]*data-children-source=\{\s*\[[\s\S]*?\{\\"file\\":\\"Example-static\.jsx\\",\\"start\\":\\"\d+:\d+\\",\\"end\\":\\"\d+:\d+\\",\\"id\\":\\"[^"]+\\"\}[\s\S]*?\]\s*\[index\]\s*\}/,
    );
    expect(output).toMatch(
      /<img[^>]*data-editor-id="[a-f0-9]{12}"[^>]*data-img-source=\{\s*\[[\s\S]*?\{\\"file\\":\\"Example-static\.jsx\\",\\"start\\":\\"\d+:\d+\\",\\"end\\":\\"\d+:\d+\\",\\"id\\":\\"[^"]+\\"\}[\s\S]*?\]\s*\[index\]\s*\}/,
    );
